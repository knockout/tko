...
- Browser automation uses `bunx playwright <cmd>` via the repo's `playwright` dependency (not `npx playwright`, not the deprecated npm package `playwright-cli`). Note: `process.md` refers to a Claude Code built-in tool also called `playwright-cli` — that is unrelated to the deprecated npm package and is not being deprecated here.
