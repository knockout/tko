describe('Binding: Foreach', function () {
  ...
  it('Should remove descendant nodes from the document (and not bind them) if the value is falsy', function () {
    testNode.innerHTML =
      "<div data-bind='foreach: someItem'><span data-bind='text: someItem.nonExistentChildProp'></span></div>"
  ...
  it('Should duplicate descendant nodes for each value in the array value (and bind them in the context of that supplied value)', function () {
    ...
    var someItems = [{ childProp: 'first child' }, { childProp: 'second child' }]
  ...
  it('Should add and remove nodes to match changes in the bound array', function () {
    testNode.innerHTML =
      "<div data-bind='foreach: { data: someItems, includeDestroyed: false }'><span data-bind='text: childProp'></span></div>"
  ...
  it('Should update all nodes corresponding to a changed array item, even if they were generated via containerless templates', function () {
    testNode.innerHTML =
      "<div data-bind='foreach: someitems'><!-- ko if:true --><span data-bind='text: $data'></span><!-- /ko --></div>"
  ...
  it('Should be able to supply show "_destroy"ed items via includeDestroyed option', function () {
    ...
    var someItems = ko.observableArray([{ childProp: 'first child' }, { childProp: 'second child', _destroy: true }])
    ko.applyBindings({ someItems: someItems }, testNode)
    expectContainHtml(
      testNode.childNodes[0],
      '<span data-bind="text: childprop">first child</span><span data-bind="text: childprop">second child</span>'
  ...
  it('Should be able to supply afterAdd and beforeRemove callbacks', function () {
    testNode.innerHTML =
      "<div data-bind='foreach: { data: someItems, afterAdd: myAfterAdd, beforeRemove: myBeforeRemove }'><span data-bind='text: $data'></span></div>"
    var someItems = ko.observableArray(['first child'])
    var afterAddCallbackData = [],
      beforeRemoveCallbackData = []
    ko.applyBindings(
      {
        someItems: someItems,
        myAfterAdd: function (elem, index, value) {
          afterAddCallbackData.push({
            elem: elem,
            index: index,
            value: value,
            currentParentClone: elem.parentNode.cloneNode(true)
          })
        },
        myBeforeRemove: function (elem, index, value) {
          beforeRemoveCallbackData.push({
            elem: elem,
            index: index,
            value: value,
            currentParentClone: elem.parentNode.cloneNode(true)
  ...
  it.skip('Should call an afterRender callback, passing all of the rendered nodes, accounting for node preprocessing and virtual element bindings', function () {
    ...
    ko.bindingProvider.instance.preprocessNode = function (node) {
      if (node.nodeType === Node.TEXT_NODE && node.data.charAt(0) === '$') {
        var newNodes = [document.createComment('ko text: ' + node.data), document.createComment('/ko')]
    ...
    testNode.innerHTML =
      "<div data-bind='foreach: { data: someItems, afterRender: callback }'><span>[</span>$data<span>]</span></div>"
  ...
  it('Exception in afterAdd callback should not cause extra elements on next update', function () {
    ...
    testNode.innerHTML =
      "<div data-bind='foreach: { data: someItems, afterAdd: callback }'><span data-bind='text: $data'></span></div>"
    var someItems = ko.observableArray(['A', 'B', 'C']),
      callback = function (element, index, data) {
        if (data === 'D') throw 'Exception'
      }
  ...
  it('Should call a beforeRemove callback function and not cause updates if an observable accessed in the callback is changed', function () {
    testNode.innerHTML =
      "<div data-bind='foreach: { data: someItems, beforeRemove: callback }'><span data-bind='text: childprop'></span></div>"
    var callbackObservable = ko.observable(1),
  ...
  it('Should call an afterMove callback function and not cause updates if an observable accessed in the callback is changed', function () {
    testNode.innerHTML =
      "<div data-bind='foreach: { data: someItems, afterMove: callback }'><span data-bind='text: childprop'></span></div>"
    var callbackObservable = ko.observable(1),
      someItems = ko.observableArray([{ childprop: 'first child' }]),
      callbacks = 0
    ko.applyBindings(
  ...
  it('Should call a beforeMove callback function and not cause updates if an observable accessed in the callback is changed', function () {
    testNode.innerHTML =
      "<div data-bind='foreach: { data: someItems, beforeMove: callback }'><span data-bind='text: childprop'></span></div>"
    var callbackObservable = ko.observable(1),
      someItems = ko.observableArray([{ childprop: 'first child' }]),
      callbacks = 0
    ko.applyBindings(
  ...
  it("Should be able to nest 'if' inside 'foreach' defined using containerless templates", function () {
    ...
    testNode.childNodes[0].childNodes[1].insertBefore(
      document.createComment('ko if: childval'),
      testNode.childNodes[0].childNodes[1].firstChild
    )
  ...
  it('Should be able to use containerless templates directly inside UL elements even when closing LI tags are omitted', function () {
    ...
    // How IE <= 8 treats it:
    testNode.innerHTML =
      "<ul><li>Header item<!-- ko foreach: someitems --><li data-bind='text: $data'><!-- /ko --></ul>"
    var viewModel = {
  ...
  it('Should be able to output HTML5 elements within container-less templates (same as above)', function () {
    ...
    if (isSupported) {
      ...
      ko.utils.setHtml(
        testNode,
        "xxx<!-- ko foreach:someitems --><div><section data-bind='text: $data'></section></div><!-- /ko -->"
      )
      var viewModel = {
        someitems: ['Alpha', 'Beta']
      }
  ...
  it('Should provide access to observable items through $rawData', function () {
    ...
    var x = ko.observable('first'),
      y = ko.observable('second'),
      someItems = ko.observableArray([x, y])
    ko.applyBindings({ someItems: someItems }, testNode)
  ...
  it('Should not clean unrelated nodes when beforeRemove callback removes some nodes before others', function () {
    ...
    try {
      testNode.innerHTML =
        "<div data-bind='foreach: {data: planets, beforeRemove: beforeRemove}'>--<span data-bind='text: name'></span>++</div>"
  ...
  describe('With "createChildContextWithAs = false" and "as"', function () {
    ...
    it('Should provide access to observable items', function () {
      testNode.innerHTML =
        "<div data-bind='foreach: { data: someItems, as: \"item\" }'><input data-bind='value: item'/></div>"
      var x = ko.observable('first'),
        y = ko.observable('second'),
        someItems = ko.observableArray([x, y])
      ko.applyBindings({ someItems: someItems }, testNode)
  ...
  describe('With "createChildContextWithAs = true" and "as"', function () {
    ...
    it('Should unwrap observable items', function () {
      testNode.innerHTML =
        "<div data-bind='foreach: { data: someItems, as: \"item\" }'><input data-bind='value: item'/><input data-bind='value: $rawData'/></div>"
      var x = ko.observable('first'),
        y = ko.observable('second'),
        someItems = ko.observableArray([x, y])
      ko.applyBindings({ someItems: someItems }, testNode)
