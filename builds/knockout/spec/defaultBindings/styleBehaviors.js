...
describe('Binding: CSS style', function() {
    ...
    it('Should give the element the specified CSS style value', function () {
        ...
        expectEqualOneOf(testNode.childNodes[0].style.backgroundColor, ["red", "#ff0000"]); // Opera returns style color values in #rrggbb notation, unlike other browsers
        myObservable("green");
        expectEqualOneOf(testNode.childNodes[0].style.backgroundColor, ["green", "#008000"]);
        myObservable(undefined);
        expect(testNode.childNodes[0].style.backgroundColor).to.deep.equal("");
    });
    ...
    it('Should be able to apply the numeric value to a style and have it converted to px', function() {
        ...
        expect(testNode.childNodes[0].style.width).to.equal("10px");
    });
    ...
    it('Should be able to apply the numeric value zero to a style', function() {
        ...
        ko.applyBindings(null, testNode);
        expectEqualOneOf(testNode.childNodes[0].style.width, ["0px", "0pt"]);
    });
    ...
    it('Should be able to apply the numeric value to a style that doesn\'t accept pixels', function() {
        ...
        ko.applyBindings(null, testNode);
        expectEqualOneOf(testNode.childNodes[0].style.zIndex, ["10", 10]);
    });
    ...
    it('Should be able to use "false" to remove a style', function() {
        ...
        ko.applyBindings(null, testNode);
        expect(testNode.childNodes[0].style.width).to.equal("");
    });
    ...
    it('Should properly respond to changes in the observable, adding px when appropriate', function() {
        ...
        width(20);
        expect(testNode.childNodes[0].style.width).to.equal("20px");
