export var dummyTemplateEngine = function (templates) {
  ...
  this.renderTemplateSource = function (templateSource, bindingContext, options, templateDocument) {
    ...
    if (typeof templateText == 'function') templateText = templateText(data, options)
...
describe('Templating', function () {
  ...
  it('Template engines can return an array of DOM nodes', function () {
    ko.setTemplateEngine(
      new dummyTemplateEngine({ x: [document.createElement('div'), document.createElement('span')] })
    )
  ...
  it('Should call a generic childrenComplete callback function', function () {
    ko.setTemplateEngine(new dummyTemplateEngine({ someTemplate: 'result = [js: childProp]' }))
    testNode.innerHTML =
      '<div data-bind=\'template: { name: "someTemplate", data: someItem }, childrenComplete: callback\'></div>'
  ...
  it('Should be able to pick template as a function of the data item using data-bind syntax, with the binding context available as a second parameter', function () {
    ...
    ko.applyBindings(
      {
        someProp: { childProp: 123, myTemplate: 'someTemplate' },
  ...
  it('Should rerender chained templates when their dependencies change, without rerendering parent templates', function () {
    ...
    var timesRenderedOuter = 0,
      timesRenderedInner = 0
    ko.setTemplateEngine(
      new dummyTemplateEngine({
        outerTemplate: function () {
          timesRenderedOuter++
          return 'outer template output, [renderTemplate:innerTemplate]'
        }, // [renderTemplate:...] is special syntax supported by dummy template engine
  ...
  it("Data binding syntax should be able to use $rawData in binding value to refer to a top level template's view model observable", function () {
    ...
    ko.setTemplateEngine(
      new dummyTemplateEngine({ someTemplate: "<div data-bind='text: ko.isObservable($rawData)'></div>" })
    )
  ...
  it('Should accept a "nodes" option that gives the template nodes', function () {
    ...
    testNode.innerHTML =
      "<div data-bind='template: { nodes: testNodes, data: testData, bypassDomNodeWrap: true }'></div>"
    var model = {
      testNodes: [document.createTextNode('begin'), document.createElement('span'), document.createTextNode('end')],
  ...
  it('Should accept a "nodes" option that gives the template nodes, and able to use the same nodes for multiple bindings', function () {
    testNode.innerHTML =
      "<div data-bind='template: { nodes: testNodes, data: testData1, bypassDomNodeWrap: true }'></div><div data-bind='template: { nodes: testNodes, data: testData2, bypassDomNodeWrap: true }'></div>"
    var model = {
      testNodes: [document.createTextNode('begin'), document.createElement('span'), document.createTextNode('end')],
  ...
  it('Should accept a "nodes" option that gives the template nodes, and it can be used in conjunction with "foreach"', function () {
    testNode.innerHTML =
      "<div data-bind='template: { nodes: testNodes, foreach: testData, bypassDomNodeWrap: true }'></div>"
  ...
  describe("Data binding 'foreach' option", function () {
    ...
    it('Should apply bindings within the context of each item in the array', function () {
      var myArray = new ko.observableArray([{ personName: 'Bob' }, { personName: 'Frank' }])
      ko.setTemplateEngine(
        new dummyTemplateEngine({ itemTemplate: "The item is <span data-bind='text: personName'></span>" })
      )
      testNode.innerHTML = '<div data-bind=\'template: { name: "itemTemplate", foreach: myCollection }\'></div>'
    ...
    it('Should handle templates in which the very first node has a binding but it does not reference any observables', function () {
      ...
      // with the array-to-DOM-node mapping state tracking
      ko.setTemplateEngine(
        new dummyTemplateEngine({ mytemplate: "<div data-bind='attr: {}'>[js: nomangle$data.name()]</div>" })
      )
      testNode.innerHTML = '<div data-bind="template: { name: \'mytemplate\', foreach: items }"></div>'
    ...
    it('Should apply bindings with an $index in the context', function () {
      var myArray = new ko.observableArray([{ personName: 'Bob' }, { personName: 'Frank' }])
      ko.setTemplateEngine(
        new dummyTemplateEngine({ itemTemplate: "The item # is <span data-bind='text: $index'></span>" })
      )
      testNode.innerHTML = '<div data-bind=\'template: { name: "itemTemplate", foreach: myCollection }\'></div>'
    ...
    it('Should accept array with "undefined" and "null" items', function () {
      ...
      ko.setTemplateEngine(
        new dummyTemplateEngine({ itemTemplate: "The item is <span data-bind='text: String($data)'></span>" })
      )
      testNode.innerHTML = '<div data-bind=\'template: { name: "itemTemplate", foreach: myCollection }\'></div>'
    ...
    it('Should update DOM nodes when a dependency of their mapping function changes', function () {
      ...
      ko.setTemplateEngine(
        new dummyTemplateEngine({ itemTemplate: '<div>The item is [js: unwrap(nomangle$data.personName)]</div>' })
      )
      testNode.innerHTML = '<div data-bind=\'template: { name: "itemTemplate", foreach: myCollection }\'></div>'
    ...
    it('Should update all child contexts and bindings when used with a top-level observable view model', function () {
      ...
      var engine = new dummyTemplateEngine({
        itemTemplate:
          "<span>The <span data-bind='text: $index'>&nbsp;</span> item <span data-bind='text: $data'>&nbsp;</span> has <span data-bind='template: { name: \"valueTemplate\", foreach: $root.itemValues[$data] }'>&nbsp;</span> </span>",
      ...
      engine.createJavaScriptEvaluatorBlock = function (script) {
        return '[[js:' + script + ']]'
      } // because we're using a binding with brackets
  ...
  it("Data binding 'templateOptions' should be passed to template", function () {
    var myModel = {
      ...
      people: new ko.observableArray([{ name: 'Alpha' }, { name: 'Beta' }])
    }
    ko.setTemplateEngine(
      new dummyTemplateEngine({
  ...
  it('Data-bind syntax should expose parent binding context as $parent if binding with an explicit \"data\" value', function () {
    ko.setTemplateEngine(
      new dummyTemplateEngine({
        myTemplate:
          "ValueLiteral: [js:$parent.parentProp], ValueBound: <span data-bind='text: $parent.parentProp'></span>"
      })
    )
  ...
  it('Data-bind syntax should expose all ancestor binding contexts as $parents', function () {
    ko.setTemplateEngine(
      new dummyTemplateEngine({
        ...
        innerTemplate:
          '(Data:[js:$data.val], Parent:[[js:$parents[0].val]], Grandparent:[[js:$parents[1].val]], Root:[js:$root.val], Depth:[js:$parents.length])'
      })
    )
  ...
  it('Should not be allowed to rewrite templates that embed control flow bindings', function () {
    ...
    ko.utils.arrayForEach(['if', 'ifnot', 'with', 'foreach', '"if"', ' with '], function (bindingName) {
      ko.setTemplateEngine(
        new dummyTemplateEngine({ myTemplate: "<div data-bind='" + bindingName + ': "SomeValue"\'>Hello</div>' })
      )
      testNode.innerHTML = '<div data-bind=\'template: { name: "myTemplate" }\'></div>'
  ...
  it('Data binding syntax should permit nested templates using virtual containers (with arbitrary internal whitespace and newlines)', function () {
    ...
    ko.applyBindings(model, testNode)
    expect(testNode.childNodes[0].innerHTML.toLowerCase()).to.contain(
      'outer <!-- ko template: \n{ name: "innertemplate" } \n'
    )
    expect(testNode.childNodes[0].innerHTML.toLowerCase()).to.contain(
      'inner via inline binding: <span data-bind="text: &quot;sometext&quot;">sometext</span>'
    )
  })
