describe('Subscribable', function () {
  ...
  it('Should be able to unsubscribe', function () {
    var instance = new ko.subscribable()
    var notifiedValue
    var subscription = instance.subscribe(function (value) {
      notifiedValue = value
    })
  ...
  it("Should be able to specify a 'this' pointer for the callback", function () {
    var model = {
      ...
      myCallback: function (arg) {
        expect(arg).to.deep.equal('notifiedValue')
        expect(this.someProperty).to.deep.equal(123)
      }
    }
    var instance = new ko.subscribable()
  ...
  it("Should be able to unsubscribe for a specific 'event'", function () {
    var instance = new ko.subscribable()
    var notifiedValue
    var subscription = instance.subscribe(
      function (value) {
        notifiedValue = value
      },
      null,
