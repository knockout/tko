...
function getWriteableValue(expression) {
    if (arrayIndexOf(javaScriptReservedWords, expression) >= 0)
        return false;
...
export function preProcessBindings(bindingsStringOrKeyValueArray, bindingOptions) {
    ...
    arrayForEach(keyValueArray, function(keyValue) {
...
export function writeValueToProperty(property, allBindings, key, value, checkIfDifferent) {
    if (!property || !isObservable(property)) {
        var propWriters = allBindings.get('_ko_property_writers');
        if (propWriters && propWriters[key])
            propWriters[key](value);
    } else if (isWriteableObservable(property) && (!checkIfDifferent || property.peek() !== value)) {
        property(value);
    }
}
