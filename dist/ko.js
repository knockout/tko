...
var canSetPrototype = _extends({}, []) instanceof Array;
...
function getDisposeCallbacksCollection(node, createIfNotFound) {
    var allDisposeCallbacks = get$1(node, domDataKey);
    if (allDisposeCallbacks === undefined && createIfNotFound) {
        allDisposeCallbacks = [];
        set$1(node, domDataKey, allDisposeCallbacks);
    }
    return allDisposeCallbacks;
}
function destroyCallbacksCollection(node) {
    set$1(node, domDataKey, undefined);
}
...
function setDomNodeChildren$1(node, childNodes) {
    if (!isStartComment(node)) setDomNodeChildren$$1(node, childNodes);else {
        emptyNode(node);
...
function simpleHtmlParse(html, documentContext) {
    ...
    var tags = stringTrim(html).toLowerCase(),
        ...
        if (this.hasSubscriptionsForEvent(event)) {
            try {
                ...
                for (var a = this._subscriptions[event].slice(0), i = 0, subscriptionInstance; subscriptionInstance = a[i]; ++i) {
                    ...
                    if (!subscriptionInstance.isDisposed) subscriptionInstance.callback(valueToNotify);
                }
            } finally {
...
subscribable.fn.limit = function limit(limitFunction) {
    var self = this,
        selfIsObservable = isObservable(self),
        ignoreBeforeChange,
        previousValue,
        pendingValue,
...
function isWriteableObservable(instance) {
    // Observable
    if (typeof instance == 'function' && instance[protoProperty$1] === observable) return true;
...
function computed(evaluatorFunctionOrOptions, evaluatorFunctionTarget, options$$1) {
    ...
    if (typeof options$$1.read != "function") throw Error("Pass a function that returns the value of the computed");
...
function isPureComputed(instance) {
    return hasPrototype(instance, computed) && instance[computedState] && instance[computedState].pure;
}
...
var operators$1 = {
  ...
  '?': function ternary(a, b) {
    return Node.value_of(a ? b.yes : b.no);
  },
...
Node.prototype.get_node_value = function () {
  ...
  return node.op(node.get_leaf_value(node.lhs), node.get_leaf_value(node.rhs));
};
...
Identifier.prototype.dereference = function (value) {
  var member,
      ...
      $data = $context.$data || {},
      last_value,
      // becomes `this` in function calls to object properties.
  i,
      n;
  ...
  // [1] See note above.
  if (typeof value === 'function' && n > 0 && last_value !== value && !last_value.hasOwnProperty(member)) {
...
Ternary.prototype.get_value = function () {
  return this;
};
...
Parser.prototype.name = function () {
  ...
  while (ch) {
    if (enclosed_by && ch === enclosed_by) {
      ...
      ch = this.next();
      if (ch !== ':' && ch !== ',') {
        this.error("Object name: " + name + " missing closing " + enclosed_by);
      }
      return name;
    } else if (ch === ':' || ch <= ' ' || ch === ',' || ch === '|') {
      return name;
...
Parser.prototype.operator = function (opts) {
  ...
  while (ch) {
    if (is_identifier_char(ch) || ch <= ' ' || ch === '' || ch === '"' || ch === "'" || ch === '{' || ch === '(' || ch === "`" || ch === ')' || ch <= '9' && ch >= '0') {
      break;
    }
...
Parser.prototype.filter = function () {
  var ch = this.next(),
      args = [],
      next_filter = function next_filter(v) {
    return v;
  },
      name = this.name();
...
Parser.prototype.expression = function (filterable) {
  ...
  while (ch) {
    ...
    if (ch === ':' || ch === '}' || ch === ',' || ch === ']' || ch === ')' || ch === '' || ch === '`' || ch === '|' && filterable === '|') {
      break;
    }
  ...
  while (ch) {
    ...
    if (!sep || sep === ',') {
      ...
      if (key.indexOf('.') !== -1) {
        ...
        ch = this.next(':');
        if (bindings[key] && _typeof(bindings[key]) === 'object' && bindings[key].constructor === Object) {
          // Extend a namespaced bindings e.g. we've previously seen
          // on.x, now we're seeing on: { 'abc' }.
          expr = this.expression(true);
          if ((typeof expr === 'undefined' ? 'undefined' : _typeof(expr)) !== 'object' || expr.constructor !== Object) {
...
Parser.prototype.convert_to_accessors = function (result) {
  ...
  objectForEach(result, function (name, _value) {
    if (_value instanceof Identifier) {
      ...
      Object.defineProperty(result, name, {
        value: function value(optionalValue, options$$1) {
          if (arguments.length === 0) {
            return _value.get_value();
          }
          if (options$$1 && options$$1.onlyIfChanged && optionalValue === _value.get_value()) {
            return;
          }
          return _value.set_value(optionalValue);
        }
      });
    } else if (_value instanceof Expression) {
      result[name] = function expressionAccessor() {
        return _value.get_value();
      };
    } else if (_value instanceof Node) {
      result[name] = function nodeAccessor() {
        return _value.get_node_value();
      };
    } else if (Array.isArray(_value)) {
      result[name] = function arrayAccessor() {
        return _value.map(function (v /*, i*/) {
          return Node.value_of(v);
        });
      };
    } else if (typeof _value !== 'function') {
      result[name] = function constAccessor() {
        return clonePlainObjectDeep(_value);
      };
    } else if (typeof _value === 'function') {
      result[name] = _value;
    }
...
Parser.prototype.parse = function (source) {
  ...
  try {
    ...
    var emsg = (typeof e === 'undefined' ? 'undefined' : _typeof(e)) === Error ? "\nMessage: <" + e.name + "> " + e.message : (typeof e === 'undefined' ? 'undefined' : _typeof(e)) === 'object' && 'at' in e ? "\n" + e.name + " " + e.message + " of \n" + "   " + e.text + "\n" + Array(e.at).join(" ") + "_/ 🔥 \\_\n" : e;
  ...
  Object.defineProperties(bindingHandlersObject, {
    'set': {
      ...
      value: function setBindingHandler(nameOrObject, value) {
        ...
        } else if ((typeof nameOrObject === 'undefined' ? 'undefined' : _typeof(nameOrObject)) === 'object') {
          if (value !== undefined) {
...
function nodeHasBindings(node) {
  if (node.nodeType === node.ELEMENT_NODE) {
    if (node.getAttribute(options.defaultBindingAttribute)) {
      return true;
    }
  } else if (node.nodeType === node.COMMENT_NODE) {
    if (options.allowVirtualElements && isStartComment(node)) {
      return true;
    }
  }

  for (var i = 0, j = this.otherProviders.length; i < j; i++) {
    if (this.otherProviders[i].nodeHasBindings(node)) {
      return true;
    }
  }
...
function preProcessBindings(bindingString) {
  ...
  arrayForEach(parseObjectLiteral(bindingString), function (keyValueItem) {
    processBinding(keyValueItem.key || keyValueItem.unknown, keyValueItem.value);
  });
...
function addProvider(p) {
  this.otherProviders.push(p);
}
function clearProviders() {
  this.otherProviders.length = 0;
}

function addBindingPreprocessor(fn) {
  this.bindingPreProcessors.push(fn);
}
function clearBindingPreprocessors() {
  this.bindingPreProcessors.length = 0;
}

function addNodePreprocessor(fn) {
  this.nodePreProcessors.push(fn);
}
function clearNodePreprocessors() {
  this.nodePreProcessors.length = 0;
}
...
function storedBindingContextForNode(node, bindingContext) {
    if (arguments.length == 2) {
        set$1(node, storedBindingContextDomDataKey, bindingContext);
        if (bindingContext._subscribable) bindingContext._subscribable._addNode(node);
    } else {
        return get$1(node, storedBindingContextDomDataKey);
    }
}
...
var LifeCycle = function () {
  ...
  LifeCycle.mixInto = function mixInto(Constructor) {
    Object.assign(Constructor.prototype || Constructor, LifeCycle.prototype);
  };

  LifeCycle.prototype.subscribe = function subscribe(observable, action) {
  ...
  LifeCycle.prototype.computed = function computed$$1(params) {
  ...
  LifeCycle.prototype.addEventListener = function addEventListener() {
  ...
  LifeCycle.prototype.anchorTo = function anchorTo(node) {
    var _this = this;
  ...
  LifeCycle.prototype.dispose = function dispose() {
  ...
  LifeCycle.prototype.addDisposable = function addDisposable(subscription) {
    var subscriptions = this[SUBSCRIPTIONS] || [];
...
var BindingHandler = function (_LifeCycle) {
    inherits(BindingHandler, _LifeCycle);

    function BindingHandler(params) {
        classCallCheck(this, BindingHandler);

        var _this = possibleConstructorReturn(this, _LifeCycle.call(this));

        var $element = params.$element,
            valueAccessor = params.valueAccessor,
            allBindings = params.allBindings,
        ...
        if (_this.onValueChange) {
            _this.subscribe(_this.computed(function () {
                return _this.value;
            }), 'onValueChange');
        }
        return _this;
    ...
    BindingHandler.registerAs = function registerAs(name) {
        var provider = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : options.bindingProviderInstance;

        provider.bindingHandlers.set(name, this);
    };

    createClass(BindingHandler, [{
        key: 'value',
        get: function get$$1() {
            return this.valueAccessor();
        },
        set: function set$$1(v) {
            return this.valueAccessor(v);
        }
    }, {
        key: 'controlsDescendants',
        get: function get$$1() {
            return false;
        }
    }], [{
...
var LegacyBindingHandler = function (_BindingHandler) {
    inherits(LegacyBindingHandler, _BindingHandler);

    function LegacyBindingHandler(params) {
        classCallCheck(this, LegacyBindingHandler);

        var _this2 = possibleConstructorReturn(this, _BindingHandler.call(this, params));

        var handler = _this2.handler;

        _this2.after = handler.after;

        if (typeof handler.dispose === 'function') {
            _this2.addDisposable(handler);
        }

        try {
            ignore(function () {
                return _this2.initReturn = handler.init && handler.init.apply(handler, toConsumableArray(_this2.legacyArgs));
            });
        } catch (e) {
        ...
        if (typeof handler.update === 'function') {
            _this2.computed(function () {
                try {
                    handler.update.apply(handler, toConsumableArray(_this2.legacyArgs));
                } catch (e) {
    ...
    createClass(LegacyBindingHandler, [{
        key: 'legacyArgs',
        get: function get$$1() {
...
function getBindingHandlerClass(handler, key, onError) {
    ...
    if (typeof handler === 'function') {
        ...
        return function (_LegacyBindingHandler) {
            ...
            function _class() {
                ...
                return possibleConstructorReturn(this, _LegacyBindingHandler.apply(this, arguments));
            }
    ...
    if ((typeof handler === 'undefined' ? 'undefined' : _typeof(handler)) === 'object') {
        return function (_LegacyBindingHandler2) {
            inherits(_class2, _LegacyBindingHandler2);

            function _class2() {
                classCallCheck(this, _class2);
                return possibleConstructorReturn(this, _LegacyBindingHandler2.apply(this, arguments));
            }

            createClass(_class2, [{
                key: 'handler',
                get: function get$$1() {
                    return Object.assign({ onError: onError }, handler);
                }
            }], [{
                key: 'allowVirtualElements',
                get: function get$$1() {
...
function applyBindingsToNodeAndDescendantsInternal(bindingContext$$1, nodeVerified, bindingContextMayDifferFromDomParentElement) {
    ...
    if (shouldApplyBindings) shouldBindDescendants = applyBindingsToNodeInternal(nodeVerified, null, bindingContext$$1, bindingContextMayDifferFromDomParentElement).shouldBindDescendants;
...
function applyBindingsToNodeInternal(node, sourceBindings, bindingContext$$1, bindingContextMayDifferFromDomParentElement) {
    ...
    var alreadyBound = get$1(node, boundElementDomDataKey);
    if (!sourceBindings) {
        if (alreadyBound) {
            onBindingError({
                during: 'apply',
        ...
        set$1(node, boundElementDomDataKey, true);
    }
    ...
    if (bindings) {
        ...
        set$1(node, 'bindingHandlers', allBindingHandlers);
        ...
        topologicalSortBindings(bindings).forEach(function (bindingKeyAndHandler) {
            function reportBindingError(during, errorCaptured) {
                onBindingError({
                    during: during, errorCaptured: errorCaptured, bindingKey: bindingKey, bindings: bindings, allBindings: allBindings,
                    bindingContext: bindingContext$$1,
                    element: node,
            ...
            var handler = bindingKeyAndHandler.handler,
                key = bindingKeyAndHandler.key,
                bindingKey = key,
            ...
            try {
                var bindingHandler = new BindingHandlerClass({
                    allBindings: allBindings,
                    $element: node,
                    $context: bindingContext$$1,
                    valueAccessor: function valueAccessor() {
                        return getValueAccessor(key).apply(undefined, arguments);
                    }
                });
...
function getBindingContext(viewModelOrBindingContext) {
    return viewModelOrBindingContext && viewModelOrBindingContext instanceof bindingContext ? viewModelOrBindingContext : new bindingContext(viewModelOrBindingContext);
}
...
function onBindingError(spec) {
    ...
    if (spec.bindingKey) {
        ...
        spec.message = "Unable to process binding \"" + spec.bindingKey + "\" in binding \"" + bindingText + "\"\nMessage: " + (error.message ? error.message : error);
    } else {
        // During: 'apply'
        error = spec.errorCaptured;
    }
...
function setDomNodeChildrenFromArrayMapping(domNode, array, mapping, options$$1, callbackAfterAddingNodes) {
    ...
    array = array || [];
    options$$1 = options$$1 || {};
    var isFirstExecution = get$1(domNode, lastMappingResultDomDataKey) === undefined;
    var lastMappingResult = get$1(domNode, lastMappingResultDomDataKey) || [];
    var lastArray = arrayMap(lastMappingResult, function (x) {
        return x.arrayEntry;
    });
    ...
    set$1(domNode, lastMappingResultDomDataKey, newMappingResult);
...
var onHandler = {
    ...
    preprocess: function preprocess(value /*, key, processBinding */) {
...
var $let = {
    init: function init(element, valueAccessor, allBindings, viewModel, bindingContext$$1) {
...
var selectExtensions = {
    ...
    readValue: function readValue(element) {
        switch (tagNameLower(element)) {
            case 'option':
                if (element[hasDomDataExpandoProperty] === true) return get$1(element, selectExtensions.optionValueDomDataKey);
                return element.value;
            case 'select':
        ...
        switch (tagNameLower(element)) {
            case 'option':
                switch (typeof value === 'undefined' ? 'undefined' : _typeof(value)) {
                    case "string":
                        set$1(element, selectExtensions.optionValueDomDataKey, undefined);
                    ...
                    default:
                        ...
                        set$1(element, selectExtensions.optionValueDomDataKey, value);
...
if (window.navigator) {
    parseVersion = function parseVersion(matches) {
        if (matches) {
...
if (ieVersion < 10) {
    ...
    var selectionChangeHandler = function selectionChangeHandler(event) {
        var target = this.activeElement,
            handler = target && get$1(target, selectionChangeHandlerName);
        if (handler) {
            handler(event);
        }
    };
    var registerForSelectionChangeEvent = function registerForSelectionChangeEvent(element, handler) {
        var ownerDoc = element.ownerDocument;
        if (!get$1(ownerDoc, selectionChangeRegisteredName)) {
            set$1(ownerDoc, selectionChangeRegisteredName, true);
            registerEventHandler(ownerDoc, 'selectionchange', selectionChangeHandler);
        }
        set$1(element, selectionChangeHandlerName, handler);
    };
}
...
var value = {
    ...
    init: function init(element, valueAccessor, allBindings) {
        ...
        var ieAutoCompleteHackNeeded = ieVersion && element.tagName.toLowerCase() == "input" && element.type == "text" && element.autocomplete != "off" && (!element.form || element.form.autocomplete != "off");
        if (ieAutoCompleteHackNeeded && arrayIndexOf(eventsToCatch, "propertychange") == -1) {
            registerEventHandler(element, "propertychange", function () {
...
var hidden = {
    update: function update(element, valueAccessor) {
        visible.update.call(this, element, function () {
            return !unwrap(valueAccessor());
        });
    }
};
...
function domElement(element) {
    ...
    this.templateType = tagNameLower$$1 === "script" ? templateScript : tagNameLower$$1 === "textarea" ? templateTextArea :
    // For browsers with proper <template> element support, where the .content property gives a document fragment
    tagNameLower$$1 == "template" && element.content && element.content.nodeType === 11 ? templateTemplate : templateElement;
}
...
domElement.prototype.data = function (key /*, valueToWrite */) {
    if (arguments.length === 1) {
        return get$1(this.domElement, dataDomDataPrefix + key);
    } else {
        set$1(this.domElement, dataDomDataPrefix + key, arguments[1]);
    }
};

var templatesDomDataKey = nextKey();
function getTemplateDomData(element) {
    return get$1(element, templatesDomDataKey) || {};
}
function setTemplateDomData(element, data$$1) {
    set$1(element, templatesDomDataKey, data$$1);
}
...
extend(templateEngine.prototype, {
    renderTemplateSource: function renderTemplateSource(templateSource, bindingContext, options$$1, templateDocument) {
        options$$1.onError("Override renderTemplateSource");
    },

    createJavaScriptEvaluatorBlock: function createJavaScriptEvaluatorBlock(script) {
        options.onError("Override createJavaScriptEvaluatorBlock");
    },

    makeTemplateSource: function makeTemplateSource(template, templateDocument) {
        // Named template
    ...
    renderTemplate: function renderTemplate(template, bindingContext, options$$1, templateDocument) {
...
function activateBindingsOnContinuousNodeArray(continuousNodeArray, bindingContext$$1) {
    ...
    if (continuousNodeArray.length) {
        ...
        invokeForEachNodeInContinuousRange(firstNode, lastNode, function (node) {
            if (node.nodeType === 1 || node.nodeType === 8) applyBindings(bindingContext$$1, node);
        });
        invokeForEachNodeInContinuousRange(firstNode, lastNode, function (node) {
...
function disposeOldComputedAndStoreNewOne(element, newComputed) {
    var oldComputed = get$1(element, templateComputedDomDataKey);
    if (oldComputed && typeof oldComputed.dispose == 'function') oldComputed.dispose();
    set$1(element, templateComputedDomDataKey, newComputed && newComputed.isActive() ? newComputed : undefined);
}
...
var template = {
    ...
    update: function update(element, valueAccessor, allBindings, viewModel, bindingContext$$1) {
        var value = valueAccessor(),
            options$$1 = unwrap(value),
            shouldDisplay = true,
            templateComputed = null,
            elseChainSatisfied = get$1(element, 'conditional').elseChainSatisfied,
            templateName;
        ...
        if (typeof options$$1 == "string") {
            ...
            if ('if' in options$$1) shouldDisplay = unwrap(options$$1['if']);
            if (shouldDisplay && 'ifnot' in options$$1) shouldDisplay = !unwrap(options$$1['ifnot']);
        }
        ...
        if ('foreach' in options$$1) {
            ...
            var innerBindingContext = 'data' in options$$1 ? bindingContext$$1.createStaticChildContext(options$$1['data'], options$$1['as']) : // Given an explitit 'data' value, we create a child binding context for it
...
nativeTemplateEngine.prototype.renderTemplateSource = function (templateSource, bindingContext, options$$1, templateDocument) {
    var useNodesIfAvailable = !(ieVersion < 9),
        ...
        init: function init(element, valueAccessor, allBindings, viewModel, bindingContext$$1) {
            ...
            computed(function () {
                var rawValue = valueAccessor(),
                    ...
                    shouldDisplayIf = !isNot !== !dataValue || isElse && rawValue === undefined,
...
function ForEach(spec) {
  ...
  this.templateNode = makeTemplateNode(spec.templateNode || (spec.name ? document.getElementById(spec.name).cloneNode(true) : spec.element));
  ...
  set$1(this.element, 'conditional', {
...
ForEach.prototype.onArrayChange = function (changeSet, isInitial) {
  ...
  if (this.changeQueue.length > 0 && !this.rendering_queued) {
    ...
    if (isInitial) {
      ...
      ForEach.animateFrame.call(window, function () {
        self.processQueue();
      });
    }
  }
...
ForEach.prototype.added = function (changeItem) {
  ...
  for (var i = 0, len = valuesToAdd.length; i < len; ++i) {
    ...
    if (pendingDelete && pendingDelete.nodesets.length) {
      ...
      applyBindingsToDescendants(this.generateContext(valuesToAdd[i]), templateClone);
...
ForEach.prototype.shouldDelayDeletion = function (data$$1) {
  return data$$1 && ((typeof data$$1 === 'undefined' ? 'undefined' : _typeof(data$$1)) === "object" || typeof data$$1 === "function");
};
...
ForEach.prototype.flushPendingDeletes = function () {
  for (var i = 0, len = this.pendingDeletes.length; i != len; ++i) {
    ...
    if (pd.data && pd.data[PENDING_DELETE_INDEX_KEY] !== undefined) delete pd.data[PENDING_DELETE_INDEX_KEY];
  }
  this.pendingDeletes = [];
};
...
function setSync(toggle) {
  if (toggle) {
    ForEach.animateFrame = function (frame) {
      frame();
    };
  } else {
    ForEach.animateFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.msRequestAnimationFrame || function (cb) {
...
var preprocessors = [interpolationMarkup, attributeInterpolationMarkup];
...
var componentBinding = {
    init: function init(element, valueAccessor, ignored1, ignored2, bindingContext$$1) {
...
function getComponentParamsFromCustomElement(node, context, parser) {
    ...
    var rawParamComputedValues = objectMap(accessors, function (paramValue /*, paramName */) {
        return computed(paramValue, null, {
            disposeWhenNodeIsRemoved: node
        });
    });

    var params = objectMap(rawParamComputedValues, function (paramValueComputed /*, paramName */) {
...
arrayForEach(["extend", "setTimeout", "arrayForEach", "arrayFirst", "arrayFilter", "arrayGetDistinctValues", "arrayIndexOf", "arrayMap", "arrayPushAll", "arrayRemoveItem", "getFormFields", "peekObservable", "postJson", "parseJson", "registerEventHandler", "stringifyJson", "range", "toggleDomNodeCssClass", "triggerEvent", "unwrapObservable", "objectForEach", "addOrRemoveItem", "setTextContent", "domData", "parseHtmlFragment", "setHtml", "compareArrays", "setDomNodeChildrenFromArrayMapping"], function (coreUtil) {
