...
import { domData, setHtml, triggerEvent, removeNode, virtualElements, options } from '@tko/utils'
...
describe('Binding: Foreach', function () {
  ...
  it('Should clean away any data values attached to the original template nodes before use', function () {
    ...
    const span = testNode.childNodes[0].childNodes[0] as HTMLSpanElement
    ...
    expect(domData.get(span, 'mydata')).to.equal(123)
    applyBindings(null, testNode)
    expect(domData.get(span, 'mydata')).to.equal(undefined)

    // Also be sure the DOM Data doesn't appear in the output
    expectContainHtml(testNode.childNodes[0], '<span></span><span></span>')
    expect(domData.get(testNode.childNodes[0].childNodes[0] as HTMLSpanElement, 'mydata')).to.equal(undefined)
    expect(domData.get(testNode.childNodes[0].childNodes[1] as HTMLSpanElement, 'mydata')).to.equal(undefined)
  })
  ...
  it('Should remove all nodes corresponding to a removed array item, even if they were added via containerless syntax and there are no other nodes', function () {
    ...
    const someitems = observableArray(['aaa', 'bbb'])
    applyBindings({ someitems: someitems }, testNode)
  ...
  it('Should update all nodes corresponding to a changed array item, even if they were generated via containerless templates', function () {
    ...
    const someitems = [observable('A'), observable('B')]
    applyBindings({ someitems: someitems }, testNode)
  ...
  it('Should be able to supply afterAdd and beforeRemove callbacks', function () {
    ...
    const someItems = observableArray(['first child'])
    let afterAddCallbackData = new Array(),
      beforeRemoveCallbackData = new Array()
    applyBindings(
      {
        someItems: someItems,
        myAfterAdd: function (elem, index, value) {
          afterAddCallbackData.push({ elem: elem, value: value, currentParentClone: elem.parentNode.cloneNode(true) })
        },
    ...
    expectContainHtml(divNode, '<span data-bind="text: $data">first child</span>')
    ...
    someItems.push('added child')
    expectContainHtml(divNode, '<span data-bind="text: $data">added child</span>')
    expect(afterAddCallbackData.length).to.equal(1)
    expect(afterAddCallbackData[0].elem).to.equal(divNode.childNodes[0])
    expect(afterAddCallbackData[0].value).to.equal('added child')
  ...
  it('Should call an afterRender callback, passing all of the rendered nodes, accounting for node preprocessing and virtual element bindings', function () {
    ...
    options.bindingProviderInstance.preprocessNode = function (node) {
      const dataNode: any = node
      if (node.nodeType === Node.TEXT_NODE && dataNode.data.charAt(0) === '$') {
        const newNodes = [document.createComment('ko text: ' + dataNode.data), document.createComment('/ko')]
    ...
    const someItems = observableArray(['Alpha', 'Beta']),
  ...
  it('Exception in afterAdd callback should not cause extra elements on next update', function () {
    ...
    const someItems = observableArray(['A', 'B', 'C']),
  ...
  it('Should not double-unwrap if the internal is iterable', function () {
    ...
    const myArrayWrapped = observable(observableArray(['data value']))
  ...
  it('Should not triple-unwrap the given value', function () {
    ...
    // See https://github.com/SteveSanderson/knockout/issues/523
    testNode.innerHTML = "<div data-bind='foreach: myArray'><span data-bind='text: $data'></span></div>"
    const myArrayWrapped = observable(observable(observableArray(['data value'])))
    applyBindings({ myArray: myArrayWrapped }, testNode)
    // Because the unwrapped value isn't an array, nothing gets rendered.
    expectContainText(testNode.childNodes[0], '')
  })
  ...
  it('Should be able to nest foreaches and access binding contexts both during and after binding', function () {
    ...
    const firstInnerTextNode = testNode.childNodes[0].childNodes[0].childNodes[1] as HTMLElement
    expect(firstInnerTextNode.nodeType).to.equal(1) // The first span associated with A1
    expect(dataFor(firstInnerTextNode)).to.equal('A1')
    expect(contextFor(firstInnerTextNode).$parent.children()[2]).to.equal('A3')
    expect(contextFor(firstInnerTextNode).$parents[1].items()[1].children()[1]).to.equal('B2')
    expect(contextFor(firstInnerTextNode).$root.rootVal).to.equal('ROOTVAL')
  })
  ...
  it("Should be able to define a 'foreach' region using a containerless template", function () {
    ...
    expect(dataFor(testNode.childNodes[3]).childprop).to.equal('second child')
    expect(contextFor(testNode.childNodes[3]).$parent.someitems.length).to.equal(2)
  })
  ...
  it("Should be able to nest 'foreach' regions defined using containerless templates", function () {
    ...
    // Verify we can access them later
    const firstInnerSpan = testNode.childNodes[3] as HTMLSpanElement
    expectContainText(firstInnerSpan, 'A1') // It is the first span bound in the context of A1
    expect(dataFor(firstInnerSpan)).to.equal('A1')
    expect(contextFor(firstInnerSpan).$parent.children()[2]).to.equal('A3')
    expect(contextFor(firstInnerSpan).$parents[1].items()[1].children()[1]).to.equal('B2')
    expect(contextFor(firstInnerSpan).$root.rootVal).to.equal('ROOTVAL')
  })
  ...
  it("Should be able to nest 'if' inside 'foreach' defined using containerless templates", function () {
    ...
    ;(testNode.childNodes[0].childNodes[1] as HTMLLIElement).innerHTML =
      "<span data-bind='text: childval.childprop'></span>"
  ...
  it('Should be able to use containerless templates directly inside UL elements even when closing LI tags are omitted', function () {
    ...
    applyBindings(viewModel, testNode)
    const match = testNode.innerHTML.toLowerCase().match(/<\/li>/g)
  ...
  it('Should provide access to observable items through $rawData', function () {
    ...
    const x = observable('first'),
      y = observable('second'),
      someItems = observableArray([x, y])
    applyBindings({ someItems: someItems }, testNode)
  ...
  it('Should not clean unrelated nodes when beforeRemove callback removes some nodes before others', function () {
    ...
    try {
      ...
      const beforeRemove = function (elem) {
      ...
      const deleted = planets.splice(3, 1)
      expectContainText(testNode, '--Mercury++--Venus++--Earth++Moon--Ceres++')
  ...
  it('Should not create a child context when `as` is used', function () {
    ...
    expect(dataFor(testNode.childNodes[0].childNodes[1])).to.equal(dataFor(testNode))
  })
