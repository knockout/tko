...
import { prepareTestNode, restoreAfter } from '../../utils/helpers/mocha-test-helpers'
...
describe('Binding: TextInput', function () {
  ...
  it('User-Agent detection for Internet Explorer', function () {
    ...
    uaList.forEach(userAgent => {
      ...
      expect(ieVersion).to.equal(userAgent['version'])

      if (userAgent['version'] == null) {
        expect(match).to.equal(null)
      } else {
        expect(match).to.not.equal(null)
        expect(MSIE_REGEX.test(userAgent['agent'])).to.equal(true)
      }
    })
  })
  ...
  it('Should assign the value to the node', function () {
    ...
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('123')
  })
  ...
  it('Should treat null values as empty strings', function () {
    ...
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('0')
  })
  ...
  it('Should assign an empty string as value if the model value is null', function () {
    ...
    applyBindings(null, testNode)
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('')
  })
  ...
  it('For observable values, should unwrap the value and update on change', function () {
    ...
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('123')
    myObservable(456)
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('456')
  })
  ...
  it("For writeable observable values, should catch the node's onchange and write values back to the observable", function () {
    ...
    expect(myObservable()).to.equal('some user-entered value')
  })
  ...
  it('For writeable observable values, when model rejects change, update view to match', function () {
    ...
    triggerEvent(testNode.children[0], 'change')
    expect(validValue()).to.equal('1234')
    expect(isValid()).to.equal(true)
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('1234')
    ...
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('1234a')
  ...
  it('Should ignore node changes when bound to a read-only observable', function () {
    ...
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('zzz')
    ...
    triggerEvent(testNode.children[0], 'change')
    expect(vm.prop).to.equal(computedValue)
    expect(computedValue()).to.equal('zzz')
  })
  ...
  it("For non-observable property values, should catch the node's onchange and write values back to the property", function () {
    ...
    triggerEvent(testNode.children[0], 'change')
    expect(model.modelProperty123).to.equal('789')
  })
  ...
  it('Should be able to write to observable subproperties of an observable, even after the parent observable has changed', function () {
    ...
    applyBindings(model, testNode)
    expect((testNode.children[0] as HTMLInputElement).value).to.equal('original value')
    ...
    expect(newSubproperty()).to.equal('Some new value')
    expect(originalSubproperty()).to.equal('original value')
  })
  ...
  it('Should write only changed values to model', function () {
    ...
    ;(testNode.children[0] as HTMLInputElement).value = '1234'
    triggerEvent(testNode.children[0], 'change')
    expect(model.writtenValue).to.equal('1234')

    // trigger change event with the same value
    model.writtenValue = undefined
    triggerEvent(testNode.children[0], 'change')
    expect(model.writtenValue).to.equal(undefined)
  })
  ...
  it('Should not write to model other than for user input', function () {
    ...
    triggerEvent(testNode.children[0], 'blur')
    expect(model.writtenValue()).to.equal(originalValue)
  ...
  if (typeof DEBUG !== 'undefined' && DEBUG) {
    ...
    describe('Event processing', function () {
      ...
      beforeEach(function () {
        ...
        options.debug = true
        restoreAfter(cleanups, bindingHandlers.textInput, '_forceUpdateOn')
      ...
      it('Should update observable asynchronously', function () {
        ...
        ;(testNode.children[0] as HTMLInputElement).value = 'some user-entered value'
        expect(myObservable()).to.equal('123') // observable is not changed yet

        clock.tick(20)
        expect(myObservable()).to.equal('some user-entered value') // it's changed after a delay
      })
      ...
      it('Should ignore "unchanged" notifications from observable during delayed event processing', function () {
        ...
        expect((testNode.children[0] as HTMLInputElement).value).to.equal('some user-entered value')
      ...
      it('Should not ignore actual change notifications from observable during delayed event processing', function () {
        ...
        expect((testNode.children[0] as HTMLInputElement).value).to.equal('some value from the server')

        // New value remains when event is processed
        clock.tick(20)
        expect(myObservable()).to.equal('some value from the server')
      })
      ...
      it('Should update model property using earliest available event', function () {
        ...
        ;(testNode.children[0] as HTMLInputElement).value = 'some user-entered value'
        triggerEvent(testNode.children[0], 'change')
        expect(model.someProp).to.equal('some user-entered value') // it's changed immediately
        expect((testNode.children[0] as any)._ko_textInputProcessedEvent).to.equal('change') // using the change event

        // even after a delay, the keydown event isn't processed
        model.someProp = undefined
        clock.tick(20)
        expect(model.someProp).to.equal(undefined)
        expect((testNode.children[0] as any)._ko_textInputProcessedEvent).to.equal('change')
      })
    })
  }
})
