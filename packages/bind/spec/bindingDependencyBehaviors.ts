...
function expectOneOfDeepEqual(actual: unknown, possibilities: unknown[]) {
  for (const expected of possibilities) {
    try {
  ...
  expect.fail(`Expected value to deep equal one of ${possibilities.length} possibilities`)
}
...
describe('Binding dependencies', function () {
  ...
  it('If the binding handler depends on an observable, invokes the init handler once and the update handler whenever a new value is available', function () {
    ...
    expect(initPassedValues.length).to.equal(1)
    expect(updatePassedValues.length).to.equal(1)
    expect(initPassedValues[0]).to.equal(undefined)
    expect(updatePassedValues[0]).to.equal(undefined)

    observable('A')
    expect(initPassedValues.length).to.equal(1)
    expect(updatePassedValues.length).to.equal(2)
    expect(updatePassedValues[1]).to.equal('A')
  })
  ...
  it('If the associated DOM element was removed by KO, handler subscriptions are disposed immediately', function () {
    ...
    expect(observable.getSubscriptionsCount()).to.equal(1)

    removeNode(testNode)

    expect(observable.getSubscriptionsCount()).to.equal(0)
  })
  ...
  it('If the binding attribute involves an observable, re-invokes the bindings if the observable notifies a change', function () {
    ...
    expect(passedValues[0]).to.equal('hello')

    observable({ message: 'goodbye' })
    expect(passedValues.length).to.equal(2)
    expect(passedValues[1]).to.equal('goodbye')
  })
  ...
  it('Should not run update before init, even if an associated observable is updated by a different binding before init', function () {
    ...
    applyBindings({ myObservable: observable }, testNode)
    expect(hasUpdatedSecondBinding).to.equal(true)
  })
  ...
  it('Should be able to get all updates to observables in both init and update', function () {
    ...
    expect(lastBoundValueInit).to.equal('initial value')
    expect(lastBoundValueUpdate).to.equal('initial value')

    // update value of observable
    vm().myProp('second value')
    expect(lastBoundValueInit).to.equal('second value')
    expect(lastBoundValueUpdate).to.equal('second value')

    // update value of observable to another observable
    vm().myProp(observableConstructor('third value'))
    expect(lastBoundValueInit).to.equal('third value')
    expect(lastBoundValueUpdate).to.equal('third value')
    ...
    expect(lastBoundValueInit).to.equal('fourth value')
    expect(lastBoundValueUpdate).to.equal('fourth value')
  })
  ...
  it('Should access latest value from extra binding when normal binding is updated', function () {
    ...
    applyBindings(vm, testNode)
    expect(updateValue).to.equal('first value')
    vm.myNonObservable = 'second value'
    observable.notifySubscribers()
    expect(updateValue).to.equal('second value')
  })
  ...
  it('Should not cause updates if an observable accessed in a childrenComplete callback is changed', function () {
    ...
    expectContainText(testNode.childNodes[0] as Element, 'child')
    ...
    bindingObservable(2)
    expectContainText(testNode.childNodes[0] as Element, 'new child')
  ...
  it('Should always use the latest value of a childrenComplete callback', function () {
    ...
    applyBindings(vm, testNode)
    sinon.assert.called(callbackSpy1)

    callbackSpy1.resetHistory()
    vm.callback = callbackSpy2

    vm.observable('new value')
    expectContainText(testNode.childNodes[0] as Element, 'new value')
    sinon.assert.notCalled(callbackSpy1)
    sinon.assert.called(callbackSpy2)
  })
  ...
  describe('Observable view models', function () {
    it('Should update bindings (including callbacks)', function () {
      ...
      expect(vm().someProp()).to.equal('some new user-entered value')
      // a click should use correct view model
      triggerEvent(testNode.childNodes[0].childNodes[1] as Element, 'click')
      expect(clickedVM).to.equal(vm())
      ...
      removeNode(testNode)
      vm(null)
      expect(vm.getSubscriptionsCount()).to.equal(0)
    })
    ...
    it("Should provide access to the view model's observable through $rawData", function () {
      ...
      expect(context.$data).to.equal('text')
    ...
    it('Should set $rawData to the view model if a function unwraps the observable view model', function () {
      ...
      expect(context.$data).to.equal('new text')
    ...
    it('Should dispose view model subscription on next update when bound node is removed outside of KO', function () {
      ...
      expect(testNode.parentElement).to.not.equal(null)
      testNode.parentNode?.removeChild(testNode)
      vm(null)
      expect(vm.getSubscriptionsCount()).to.equal(0)
    })
    ...
    it('Should maintain correct $rawData in extended context when parent is bound to a function that returns an observable view model', function () {
      ...
      expect(parentContext.$data).to.equal('vm1')
      expect(parentContext.$rawData).to.equal(vm1)

      expect(childContext).to.not.equal(parentContext)
      expect(childContext.$data).to.equal('vm1')
      expect(childContext.$rawData).to.equal(vm1)

      // Updating view model updates bindings and context
      whichVm(vm2)
      expectContainText(testNode, 'vm2')
      expect(childContext.$data).to.equal('vm2')
  ...
  describe('Order', function () {
    ...
    it('Should leave bindings without an "after" value where they are (extended)', function () {
      ...
      applyBindings(null, testNode)
      expectOneOfDeepEqual(bindingOrder, [
        [1, 2, 3],
        [3, 1, 2]
      ])
    })
