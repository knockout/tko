...
describe('Parse HTML fragment', function () {
  ...
  arrayForEach(
    [
      { html: '<tr-component></tr-component>', parsed: ['<tr-component></tr-component>'], OldjQueryFails: true },
      {
        html: '<thead><tr><th><thcomponent>hello</thcomponent></th></tr></thead>',
        parsed: ['<thead><tr><th><thcomponent>hello</thcomponent></th></tr></thead>']
      },
      {
      ...
      {
        html: '<!-- ko if:true --><tr></tr><!-- /ko -->',
        parsed: ['<!-- ko if:true -->', '<tr></tr>', '<!-- /ko -->']
      },
      ...
      {
        html: '<optgroup label=x><option>text</option></optgroup>',
        parsed: ['<optgroup label=x><option>text</option></optgroup>']
      },
      ...
      {
        html: '<!-- v --><thead></thead><!-- vv -->',
        parsed: ['<!-- v -->', '<thead></thead>', '<!-- vv -->'],
        simpleParserFails: true
      }
    ],
    function (data) {
      it('should parse ' + data.html + ' correctly', function () {
        ...
        if (!supportsTemplateTag) {
          if (!window.jQuery && data.simpleParserFails) {
            return
          }
          if (window.jQuery && parseFloat(window.jQuery.fn.jquery[0]) < 3 && data.OldjQueryFails) {
            return
          }
        }
  ...
  it('template is to long', function () {
    if (options.templateSizeLimit === 0) return
  ...
  it('template contains script-tag', function () {
    ...
    htmlArray.push(
      '<div><i><script           type="application/javascript"    >console.log(\'oups\')</script    ></i></div>'
    )
