...
function executeTemplate (targetNodeOrNodeArray, renderMode, template, bindingContext$$1, options, afterBindingCallback) {
  options = options || {};
  var firstTargetNode = targetNodeOrNodeArray && getFirstNodeFromPossibleArray(targetNodeOrNodeArray);
  var templateDocument = (firstTargetNode || template || {}).ownerDocument;
  var templateEngineToUse = (options.templateEngine || _templateEngine);
  var renderedNodesArray = templateEngineToUse.renderTemplate(template, bindingContext$$1, options, templateDocument);
  ...
  if (haveAddedNodesToParent) {
    ...
    if (options.afterRender) { ignore(options.afterRender, null, [renderedNodesArray, bindingContext$$1['$data']]); }
  }
...
class TemplateBindingHandler extends AsyncBindingHandler {
  ...
  onValueChange () {
    ...
    if ('foreach' in options) {
      ...
      templateComputed = renderTemplateForEach(templateName || element, dataArray, options, element, bindingContext$$1, this.completeBinding);

      elseChainSatisfied((unwrap(dataArray) || []).length !== 0);
    } else if (shouldDisplay) {
      // Render once for this single data point (or use the viewModel if no data was provided)
      var innerBindingContext = ('data' in options)
        ? bindingContext$$1.createStaticChildContext(options.data, options.as)  // Given an explitit 'data' value, we create a child binding context for it
...
var index = {
    ...
    Component: registry.ComponentABC,
    // --- Utilities ---
    cleanNode: cleanNode,
    memoization: memoization,
    removeNode: removeNode,
