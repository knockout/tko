...
var submit = {
    init: function (element, valueAccessor, allBindings, viewModel, bindingContext$$1) {
        ...
        registerEventHandler(element, "submit", function (event) {
            ...
            try { handlerReturnValue = value.call(bindingContext$$1['$data'], element); }
            finally {
                if (handlerReturnValue !== true) { // Normally we want to prevent default action. Developer can override this be explicitly returning true.
                    if (event.preventDefault)
                        event.preventDefault();
...
// element was changed.
if (ieVersion < 10) {
    var selectionChangeRegisteredName = data.nextKey(),
        selectionChangeHandlerName = data.nextKey();
    var selectionChangeHandler = function(event) {
        var target = this.activeElement,
            handler = target && data.get(target, selectionChangeHandlerName);
        if (handler) {
            handler(event);
        }
    };
...
let templateComputedDomDataKey = data.nextKey();
...
class TemplateBindingHandler extends AsyncBindingHandler {
  ...
  disposeOldComputedAndStoreNewOne (element, newComputed) {
    let oldComputed = data.get(element, templateComputedDomDataKey);
...
class ConditionalBindingHandler extends AsyncBindingHandler {
  constructor (params) {
    ...
    data.set(this.$element, 'conditional', { elseChainSatisfied });
...
class ForEachBinding extends AsyncBindingHandler {
  ...
  constructor (params) {
    ...
    data.set(this.$element, 'conditional', {
...
class ComponentBinding extends AsyncBindingHandler {
  ...
  computeApplyComponent () {
    ...
    index$1.get(componentName, (defn) => this.applyComponentDefinition(componentName, componentParams, defn));
  }
...
//# sourceMappingURL=tko.es6.js.map
