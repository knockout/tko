...
import { bindingHandlers } from '../index.js'

import * as expressionRewriting from '../src/expressionRewriting.js'


ddescribe('Expression Rewriting', function() {

    it('Should be able to parse simple object literals', function() {
        var result = expressionRewriting.parseObjectLiteral("a: 1, b: 2, \"quotedKey\": 3, 'aposQuotedKey': 4");
    ...
    it('Should ignore any outer braces', function() {
        var result = expressionRewriting.parseObjectLiteral("{a: 1}");
    ...
    it('Should be able to parse object literals containing string literals', function() {
        var result = expressionRewriting.parseObjectLiteral("a: \"comma, colon: brace{ bracket[ apos' escapedQuot\\\" end\", b: 'escapedApos\\\' brace} bracket] quot\"'");
    ...
    it('Should be able to parse object literals containing child objects, arrays, function literals, and newlines', function() {
        ...
        var result = expressionRewriting.parseObjectLiteral(
    ...
    it('Should be able to parse object literals containing division and regular expressions', function() {
        var result = expressionRewriting.parseObjectLiteral("div: null/5, regexpFunc: function(){var regex=/{/g;return /123/;}");
        expect(result.length).toEqual(2);
    ...
    it('Should parse a value that begins with a colon', function() {
        var result = expressionRewriting.parseObjectLiteral("a: :-)");
        expect(result.length).toEqual(1);
        expect(result[0].key).toEqual("a");
    ...
    it('Should be able to cope with malformed syntax (things that aren\'t key-value pairs)', function() {
        var result = expressionRewriting.parseObjectLiteral("malformed1, 'mal:formed2', good:3, {malformed:4}, good5:5, keyonly:");
    ...
    it('Should ensure all keys are wrapped in quotes', function() {
        var rewritten = expressionRewriting.preProcessBindings("a: 1, 'b': 2, \"c\": 3");
    ...
    it('(Private API) Should convert writable values to property accessors', function () {
        ...
        bindingHandlers.a = { twoWayBinding: true }
        bindingHandlers.b = { twoWayBinding: true }
        bindingHandlers.c = { twoWayBinding: true }
        bindingHandlers.d = { twoWayBinding: true }
        bindingHandlers.e = { twoWayBinding: true }
        bindingHandlers.f = { twoWayBinding: true }
        bindingHandlers.g = { twoWayBinding: true }
        bindingHandlers.h = { twoWayBinding: true }
        bindingHandlers.i = { twoWayBinding: true }
        bindingHandlers.j = { twoWayBinding: true }

        var rewritten = expressionRewriting.preProcessBindings(
        ...
        bindingHandlers.a = { twoWayBinding: false }
        bindingHandlers.b = { twoWayBinding: false }
        bindingHandlers.c = { twoWayBinding: false }
        bindingHandlers.d = { twoWayBinding: false }
        bindingHandlers.e = { twoWayBinding: false }
        bindingHandlers.f = { twoWayBinding: false }
        bindingHandlers.g = { twoWayBinding: false }
        bindingHandlers.h = { twoWayBinding: false }
        bindingHandlers.i = { twoWayBinding: false }
        bindingHandlers.j = { twoWayBinding: false }
        ...
        expect(parsed.g).toEqual(model.assistant);
    ...
    it('Should be able to eval rewritten literals that contain unquoted keywords as keys', function() {
        var rewritten = expressionRewriting.preProcessBindings("if: true");
    ...
    it('Should return accessor functions for each value when called with the valueAccessors option', function() {
        var rewritten = expressionRewriting.preProcessBindings("a: 1", {valueAccessors:true});
    ...
    it('Should be able to parse and evaluate object literals containing division', function() {
        ...
        var result = expressionRewriting.parseObjectLiteral("a: null/1, b: 2/1, c: (6) / 2, d: '2'/2, r: /a regex/");
        expect(result).toEqual([{key:'a', value: 'null/1'}, {key: 'b', value: '2/1'}, {key: 'c', value: '(6)/2'}, {key: 'd', value: '\'2\'/2'}, {key: 'r', value: '/a regex/'}]);
        var rewritten = expressionRewriting.preProcessBindings(result, {valueAccessors:true});
        var evaluated = eval("({" + rewritten + "})");
