...
- **Binding lifecycle (`descendantsComplete` / `completeOn`) follows Knockout 3.5.** `childrenComplete: fn` fires when a node's direct children (re-)bind; `descendantsComplete: fn` fires once children *and every async descendant* (components, conditionals) have bound. `descendantsComplete` **re-fires per content cycle** — a nested `if` becoming true, or toggling a conditional off then on, fires it again, so don't assume it's one-shot. `completeOn: "render"` on an inner `if`/`ifnot`/`with` holds an ancestor's `descendantsComplete` (or a component's `koDescendantsComplete`) open until that inner content actually renders. `completeOn` is a reserved binding key. A component viewmodel's `koDescendantsComplete(el)` fires via the element's `descendantsComplete` event, so inner components complete before outer ones.
