...
### Example
```tsx
const numberOfClicks = ko.observable(0)
const incrementClickCounter = () => numberOfClicks(numberOfClicks() + 1)

<div>
  You've clicked <span ko-text={numberOfClicks} /> times
  <button ko-click={incrementClickCounter}>Click me</button>
</div>
```
...
```tsx
const places = ko.observableArray(['London', 'Paris', 'Tokyo'])
const removePlace = place => places.remove(place)

<ul ko-foreach={places}>
  <li><span ko-text="$data" /> <button ko-click={removePlace}>Remove</button></li>
</ul>
```
...
<button ko-click={myFunction}>Click me</button>
```
...
```tsx
const myFunction = (param1, param2, data, event) => {
  // do something with the provided values
}

<button ko-click={(data, event) => myFunction('param1', 'param2', data, event)}>Click me</button>
```
...
<button ko-click={myFunction.bind(null, 'param1', 'param2')}>
  Click me
</button>
```
...
<div ko-click={myDivHandler}>
  <button ko-click={myButtonHandler} ko-clickBubble={false}>
    Click me
  </button>
</div>
```
