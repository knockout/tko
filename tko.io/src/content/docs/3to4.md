...
### `descendantsComplete` matches Knockout 3.5

In Knockout 3.5, `descendantsComplete` fires after every descendant — including content rendered asynchronously by a nested `if`, `template`, `foreach`, or component — has finished binding. Knockout tracks this with an ancestor-linked async-completion context and, on control-flow bindings, a `completeOn: 'render'` option.

TKO ports that mechanism, so `descendantsComplete` behaves as it did in Knockout 3.5:

- It fires once a node's children **and every asynchronously-completing descendant** have bound.
- It **re-fires per content cycle**: if a descendant `if`/`with` renders its content after the fact (for example when its condition flips to `true`), the callback fires again. Attach one-shot callbacks accordingly.
- `completeOn: "render"` on an inner `if`/`ifnot`/`with` holds the ancestor's `descendantsComplete` (and a component's `koDescendantsComplete`) open until that inner content actually renders.

```html
<!-- Fires onReady after the nested async content has rendered -->
<div data-bind="if: ready, descendantsComplete: onReady">
  <div data-bind="if: innerReady">…</div>
</div>

<!-- Defer onReady until the inner content renders -->
<div data-bind="if: ready, descendantsComplete: onReady">
  <div data-bind="if: innerReady, completeOn: 'render'">…</div>
</div>
```

`childrenComplete` remains available for the narrower "this node's direct children were (re-)bound" signal, and is notified every render cycle.

> **Earlier 4.x behavior:** releases up to v4.1.0 decided `descendantsComplete` once, synchronously, when the node itself bound, and did not re-fire when descendant async content rendered later. If you added workarounds for that gap (moving the callback onto the inner node via `childrenComplete`), you can revert them.
